...
test("Deve modificar a senha de um usuário", async function() {
  const user = new User("John Doe", "john.doe@gmail.com", "abc123456")
  user.updatePassword('asd123456')
  expect(user.getPassword()).toBe("asd123456")
})

test("Não deve modificar a senha do usuário se ela não tiver no mínimo 8 caracteres", async function() {
  const user = new User("John Doe", "john.doe@gmail.com", "abc123456")
  expect(() => user.updatePassword('asd')).toThrow(new Error("Minimum length is 8"))
})
