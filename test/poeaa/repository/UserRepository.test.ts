...
test("Deve salvar um novo usuário", async function () {
  ...
  const user = new User("John Doe", email, "abc213456")
  ...
  expect(savedUser.getPassword()).toBe("abc213456")
...
test("Deve atualizar um usuário", async function () {
  const email = `john.doe${Math.random()}@gmail.com`
  const user = new User(`john.doe${Math.random()}`, email, "abc213456")
  const userRepository = new UserRepositoryDatabase()
  await userRepository.save(user)
  const savedUser = await userRepository.getByEmail(email)
...
test("Deve listar três usuários", async function () {
  const userRepository = new UserRepositoryDatabase()
  await userRepository.save(new User("John Doe", `john.doe1@gmail.com`, "abc213456"))
  await userRepository.save(new User("John Doe", `john.doe2@gmail.com`, "abc213456"))
  await userRepository.save(new User("John Doe", `john.doe3@gmail.com`, "abc213456"))
